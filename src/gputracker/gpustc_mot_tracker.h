...
public:
	BinaryMatching() { // @suppress("Class members should be properly initialized")
	}
	;
	~BinaryMatching() {
	}
...
private:
	...
	void complexOperation(const Mat src1, const Mat src2, Mat &dst,
			int flag = 0);
	void complexOperation_cpu(const Mat src1, const Mat src2, Mat &dst,
				int flag = 0);
	void complexOperation_gpu(const Mat src1, const Mat src2, Mat &dst,
				int flag = 0);
