...
void STC_MOT_Tracker::process_key_frame(const vector<Rect> &in_det_box,
	...
	if (last_key_frame_offset >= 0)
		update_vehicle_location(last_key_frame_offset, det_box);
	if (display_config_) {
		printf("**track:\n");
		for (int v_idx = 0; v_idx < frame_[frame_.size() - 1].obj.size();
				v_idx++)
	...
	add_new_vehicle(det_box, det_type);
	if (display_config_) {
		if (frame_.size() > 1)
...
float STC_MOT_Tracker::calc_img_dis(const Rect b1, const Rect b2, float &dx, // @suppress("No return")
		float &dy) {
...
// -------------------------------------- STC Tracker -----------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------
STCTracker::STCTracker() { // @suppress("Class members should be properly initialized")
...
void begintime() {
	...
	pre_time = (start.tv_sec) * 1000000 + start.tv_usec;
}

void endtime() {
	struct timeval end;
	gettimeofday(&end, NULL);
	unsigned long long tt2 = (end.tv_sec) * 1000000 + end.tv_usec;
	sum_time += tt2 - pre_time;
	time_count += 1;
	cout << "TTTTT:" << time_count << "   " << sum_time / 1000 << endl;
}
...
//ofstream testfile("u1");
//void STCTracker::complexOperation(const Mat src1, const Mat src2, Mat &dst,
//		int flag) {
//
//	complexOperation_gpu(src1,src2,dst,flag);
//
//}

//ofstream testfile("u2");
void STCTracker::complexOperation(const Mat src1, const Mat src2, Mat &dst,
		int flag) {
	complexOperation_cpu(src1, src2, dst, flag);
}

int mcount = 0;
GPUAllocator* _allocator = new GPUAllocator(1024 * 1024 * 4096);

// Define two complex-value operation
void STCTracker::complexOperation_cpu(const Mat src1, const Mat src2, Mat &dst,
		int flag) {
//	starttime();
...
//	gsrc1.upload(src1);
//	gsrc2.upload(src2);
//	gdst.create(src1.rows, src1.cols, CV_64FC2);
...
//	}
	...
	if (flag) {
		Mat dibu = double(1.0)
				/ (B_Real.mul(B_Real) + B_Imag.mul(B_Imag) + Scalar(0.000001));
...
//	for (int i = 0; i < A_Real.rows; i++) {
//		for (int j = 0; j < A_Real.cols; j++) {
//			testfile << R_Imag.at<double>(i, j,0) << endl;
////			testfile << dst.at<double>(i, j,1) << endl;
//		}
//	}
...
// Define two complex-value operation
void STCTracker::complexOperation_gpu(const Mat src1, const Mat src2, Mat &dst,
		int flag) {
//	starttime();
	CV_Assert(src1.size == src2.size);
	CV_Assert(src1.channels() == 2);

	cv::cuda::GpuMat gsrc1(_allocator), gsrc2(_allocator), gdst(_allocator);

//	if(mcount>100){

//	}
	gsrc2.create(src1.rows, src1.cols, CV_64FC2);
...
//	}
	...
	gsrc1.create(src1.rows, src1.cols, CV_64FC2);
	gdst.create(src1.rows, src1.cols, CV_64FC2);

	gsrc1.upload(src1);
	gsrc2.upload(src2);

	cv::cuda::GpuMat A_Real(_allocator), A_Imag(_allocator), B_Real(_allocator),
			B_Imag(_allocator), R_Real(_allocator), R_Imag(_allocator);

	A_Real.create(src1.rows, src1.cols, CV_64FC1);
	A_Imag.create(src1.rows, src1.cols, CV_64FC1);
	B_Real.create(src1.rows, src1.cols, CV_64FC1);
	B_Imag.create(src1.rows, src1.cols, CV_64FC1);
	R_Real.create(src1.rows, src1.cols, CV_64FC1);
	R_Imag.create(src1.rows, src1.cols, CV_64FC1);

	vector<cv::cuda::GpuMat> planesA;
	planesA.push_back(A_Real);
	planesA.push_back(A_Imag);
	cv::cuda::split(src1, planesA);

//	planes[0].copyTo(A_Real);
//	planes[1].copyTo(A_Imag);

	vector<cv::cuda::GpuMat> planesB;
	planesB.push_back(B_Real);
	planesB.push_back(B_Imag);
	cv::cuda::split(src2, planesB);
//	planes[0].copyTo(B_Real);
//	planes[1].copyTo(B_Imag);

//	cout<<"HUU"<<B_Real.cols<<endl;
//	dst.create(src1.rows, src1.cols, CV_64FC2);
	vector<cv::cuda::GpuMat> planesR;
	planesR.push_back(R_Real);
	planesR.push_back(R_Imag);
	cv::cuda::split(gdst, planesR);
//	R_Real = planesR[0];
//	R_Imag = planesR[1];
...
//	}
	...
	cv::cuda::GpuMat temp(_allocator);
	temp.create(src1.rows, src1.cols, CV_64FC1);
	cv::cuda::GpuMat temp2(_allocator);
	temp2.create(src1.rows, src1.cols, CV_64FC1);
...
//				/ (B_Real.mul(B_Real) + B_Imag.mul(B_Imag) + Scalar(0.000001));
		...
		cv::cuda::multiply(B_Real, B_Real, temp);
		cv::cuda::multiply(B_Imag, B_Imag, temp2);
		cv::cuda::add(temp, temp2, temp);
		cv::cuda::add(temp, Scalar(0.000001), temp);
		cv::cuda::divide(Scalar(1.0), temp, temp);

//		cv::cuda::GpuMat dibu;
//		dibu.create(src1.rows, src1.cols, CV_64FC1);

//		cv::cuda::GpuMat dibu = double(1.0)
//				/ (temp + temp2 + Scalar(0.000001));

		cv::cuda::multiply(A_Real, B_Real, R_Real);
		cv::cuda::multiply(A_Imag, B_Imag, temp2);
		cv::cuda::add(R_Real, temp2, R_Real);
		cv::cuda::multiply(R_Real, temp, R_Real);

		cv::cuda::multiply(A_Imag, B_Real, R_Imag);
		cv::cuda::multiply(A_Real, B_Imag, temp2);
		cv::cuda::subtract(R_Imag, temp2, R_Imag);
		cv::cuda::multiply(R_Imag, temp, R_Imag);
...
//		R_Imag = (A_Imag.mul(B_Real) - A_Real.mul(B_Imag)).mul(dibu);
		...
		cv::cuda::multiply(A_Real, B_Real, temp);
		cv::cuda::multiply(A_Imag, B_Imag, temp2);
		cv::cuda::subtract(temp, temp2, R_Real);

		cv::cuda::multiply(A_Imag, B_Real, temp);
		cv::cuda::multiply(A_Real, B_Imag, temp2);
		cv::cuda::add(temp, temp2, R_Imag);
...
//	for (int i = 0; i < A_Real.rows; i++) {
//		for (int j = 0; j < A_Real.cols; j++) {
//			testfile << R_Imag.at<double>(i, j) << endl;
//		}
//	}
//    struct timeval end;
//    gettimeofday(&end, NULL);
...
//	Mat cputemp;
//	R_Imag.download(cputemp);
//	for (int i = 0; i < A_Real.rows; i++) {
//		for (int j = 0; j < A_Real.cols; j++) {
//			testfile << cputemp.at<double>(i, j) << endl;
//		}
//	}

//	cout<<"size:"<<gdst.rows<<"|"<<gdst.cols<<endl;

//	endtime();
}

Mat IMatrix(321, 321, CV_64FC1), JMatrix(321, 321, CV_64FC1);
bool initial_matrix = false;
...
//	}
	...
	int cx = center.x - cxtRegion.x;
	int cy = center.y - cxtRegion.y;
	double sigma2 = -1.0 / (2 * sigma * sigma);
	double alphasqt = -1.0 / sqrt(alpha);
//	Mat negdistpow2(cxtRegion.size(), CV_64FC1);
//	Mat dist(cxtRegion.size(), CV_64FC1);
	...
	if (!initial_matrix) {
		for (int i = 0; i < 321; i++) {
			for (int j = 0; j < 321; j++) {
				IMatrix.at<double>(i, j) = i;
				JMatrix.at<double>(i, j) = j;
			}
		}
		initial_matrix = true;
	}
	...
	Mat imatrix(IMatrix, Rect(0, 0, cxtRegion.width, cxtRegion.height));
	Mat jmatrix(JMatrix, Rect(0, 0, cxtRegion.width, cxtRegion.height));

	cxtPriorPro = double(cx) - jmatrix;
	cxtPriorPro = cxtPriorPro.mul(cxtPriorPro);
	cxtPosteriorPro = double(cy) - imatrix;
	cxtPosteriorPro = cxtPosteriorPro.mul(cxtPosteriorPro);

	cxtPriorPro += cxtPosteriorPro;
	cv::sqrt(cxtPriorPro, cxtPosteriorPro);
	cxtPosteriorPro.convertTo(cxtPosteriorPro, -1, alphasqt);
	cxtPriorPro.convertTo(cxtPriorPro, -1, sigma2);

//	for (int i = 0; i < cxtRegion.height; i++) {
//		for (int j = 0; j < cxtRegion.width; j++) {
//			double c=(cx-j)*(cx-j)+(cy-i)*(cy-i);
//			cxtPriorPro.at<double>(i, j)=c*sigma2;
//			cxtPosteriorPro.at<double>(i, j)=sqrt(c)*alphasqt;
//		}
//	}

	cv::exp(cxtPriorPro, cxtPriorPro);
	sum_prior = cv::sum(cxtPriorPro).val[0];

	cv::exp(cxtPosteriorPro, cxtPosteriorPro);
	sum_post = cv::sum(cxtPosteriorPro).val[0];

//	for (int i = 0; i < cxtRegion.height; i++) {
//		for (int j = 0; j < cxtRegion.width; j++) {
//			testfile << cxtPriorPro.at<double>(i, j) << endl;
//		}
////		testfile ;
//	}
